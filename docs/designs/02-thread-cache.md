...
| `RequestGlobalTrim` / `HandleGlobalTrimRequest` | `static void ...` / `void ...`                  | publish/consume cooperative epoch；只在慢路径或 safepoint 生效                                                           | ❌        |
...
每个尺寸类的高水位配额 `max_size_` 与连续溢出计数 `overages_`（见 §3）驱动一个「慢上车 → 高水位 → 慢下车」的状态机：增长由 `quota_policy::NextAfterRefill` 决定，衰减由 `quota_policy::NextAfterOverflow` 决定，二者均为无 I/O 的纯函数（`include/ammalloc/thread_cache.h` 的 `quota_policy` 命名空间），可被
...
- hard trim 选择 direct bitmap release；`ReleaseAll` 保持普通 TransferCache reuse，避免多线程同时退出时将 TLS destructor 变成 bitmap/PageCache purge storm。需要 RSS 回收的 worker 应在退出前显式调用 `am_thread_cache_purge()`；已有 TransferCache retention 再由CentralCache drain 处理。

### 6.5 协作式收缩（Cooperative Pressure Request）与 Trim

ThreadCache 是线程私有的，控制方（purge API / 后台治理器）**永远不能直接操作他人的 TLS FreeList**——那会对无锁结构产生非法跨线程写。因此收缩采用「发布-订阅 +自我执行」协作模型：控制方只在共享公告板上发布一个**意图**，每个线程在自己的慢路径安全点上读取公告、对**自己的**缓存执行收缩。普通 local hit 完全不读公告板，协作机制不向热路径引入任何共享 cache line。请求不是对完全 idle 线程的中断机制，后者必须由上层 cooperative scheduler 处理（见 §4 GC 边界）。

**公告板** **`trim_request_`（`inline static std::atomic<uint64_t>`）**

全进程唯一、初值 0，被所有 ThreadCache 实例共享；编码 `(epoch << 1) | mode`：

- 高 63 位是 **epoch**（请求代际号），低 1 位是收缩**模式**（`kReuse` 软 /`kRelease` 硬）；

- 必须**打包进单个原子**：若拆成两个字段并发发布，读者可能拼出「甲请求的 epoch 乙请求的 mode」这种不存在的组合；单原子使 `{epoch, mode}` 作为不可分割的值整体可见。该状态在普通 ThreadCache hit 上从不被读取。

**epoch 的含义（generation / era 代际号）**

epoch 不是时间戳，而是「第几代收缩请求」的**单调递增计数器**（初值 0）。它存在的意义是让观察者区分**新旧**——只有 mode 的公告板无法回答「这个请求我见过吗」。引入 epoch 后，每线程用私有 `observed_trim_epoch_`（初值 0）记录「已办到第几代」：

- **幂等**：`epoch == observed_trim_epoch_` 表明本代已处理，同一请求在之后的任意多个安全点只执行一次；
...
CAS 循环把公告板写为新值 `((observed >> 1) + 1) << 1 | mode`：

- `next_epoch = 当前 epoch + 1` 单调递增，多发布方并发时每个请求仍获得唯一且全序的代际号，不会互相覆盖丢请求；

- 写侧 `memory_order_release`，保证请求对观察者立即可见。

**执行端** **`HandleGlobalTrimRequest()`**

挂在两个慢路径入口的第一行（`FetchFromCentralCache` 与 `DeallocateSlowPath`）：

1. `acquire` 读公告板，取 `epoch = 值 >> 1`；
2. 若 `epoch == observed_trim_epoch_`，本代已处理，直接返回（幂等）；
3. 否则按低 1 位 `mode` 调用 `Trim(mode, TrimTargetForMode(mode))`，再把`observed_trim_epoch_`更新为 `epoch`；此后原慢路径继续（refill 照常取货、overflow 照常归还一个 batch），互不干扰。

选慢路径做安全点，是因为快路径连一次全局原子读都负担不起；收缩等于「顺路」完成，与 §6.4「预算只在慢路径记账」是同构取舍。

**响应语义：不是所有线程都会真正收缩**

一个请求发布后，各线程的响应按状态分为四档，实际归还对象的只是其中一小部分：

| 线程状态                 | 观察结果                            | 实际动作                                          |
| -------------------- | ------------------------------- | --------------------------------------------- |
| 已处理过该 epoch（再次经过安全点） | `epoch == observed_trim_epoch_` | 幂等返回，零成本空跑                                    |
| 未处理、但缓存已低于 target    | 发现新请求，调 Trim                    | `cached_bytes ≤ target_bytes`，内部空跑（Trim 单向收缩） |
| 未处理、且存在超额缓存          | 发现新请求                           | 真正驱逐对象并收缩配额                                   |
| 完全 idle（不进慢路径）       | 不读公告板                           | 不响应，留待显式 safepoint                            |

- 每个 epoch 在每个线程上**至多执行一次 Trim**：首次经过安全点时处理，此后命中幂等分支。

- 多数线程实际是空跑或不响应；真正归还对象的是「未处理过 + 有超额缓存」的活跃线程子集。

- idle 线程不响应是设计决策，不是疏漏（原因展开见下）。

**idle 线程为何不响应 trim**

协作模型对完全 idle 线程「放行」，有四个递进的原因：
...
| 配额收敛到  | `max(batch, size)`             | `max(1, size)`（清空后回到起始配额 1）                 |
| 典型场景   | 日常治理：大幅瘦身但留余家                  | purge / 退出前：彻底放开占用                          |

Trim 的三条硬性语义：

- **单向收缩、绝不扩张**：`cached_bytes ≤ target_bytes` 时无事发生；软模式不会为建立暖基线而让冷类增长——否则 L4 可绕过 L3 的预算否决（见 §6.7 常驻不变量）。
...
`GetThreadCacheStats()` 提供 aggregate `reserved_quota_bytes`、trim count、trim 前观察到的 cached-object bytes、trimmed bytes 与 budget-denied growth；CentralCache 的 `GetStats()`提供 transfer drained bytes 和 direct release 解钉 Span 的计数。这些都是 slow-pathtelemetry：不能将 cached-object bytes 或 Span count 直接解释成 RSS。
...
[include/ammalloc/size\_class.h](../../include/ammalloc/size_class.h)、
[include/ammalloc/free\_list.h](../../include/ammalloc/free_list.h)、
[include/ammalloc/thread\_cache.h](../../include/ammalloc/thread_cache.h) 的编译期常量推导，
修改任一旋钮后必须重新核算本表）。
...
| `SizeClass::kNumSizeClasses`                | 40                                                    | `SizeClass`（size\_class.h）      | `free_lists_` 数组长度 = 参与配额预约的类别数                       |
| `SizeClass::kMaxBatchSize`                  | 512                                                   | `SizeClass`（size\_class.h）      | batch 上界截断，防小对象类单次搬运过大                                |
| `batch`                                     | `clamp(MAX_TC_SIZE / class_size, 2, 512)`             | `SizeClass::CalculateBatchSize` | 配额与搬运的基本单位（对象个数）                                      |
| `quota_policy::kMaxQuotaBatches`            | 8                                                     | `quota_policy`（thread\_cache.h） | 单类配额封顶 = 8 × batch                                    |
| `quota_policy::kMaxOverages`                | 3                                                     | `quota_policy`（thread\_cache.h） | 连续 3 次溢出触发一次「降一个 batch」的衰减                            |
| `FreeList::max_size_` 初值 / 下限               | 1 / 1                                                 | `FreeList`（free\_list.h）        | slow-start 起点；下限 1 防止 refill 永久失败                     |
| `FreeList::overages_`                       | 0 起步，成功 refill 清零                                     | `FreeList`（free\_list.h）        | 衰减信号：新需求取消衰减趋势                                        |
| `ThreadCache::kDefaultCacheBudgetBytes`     | 2 MiB                                                 | `ThreadCache`（thread\_cache.h）  | 单线程 aggregate quota-capacity ceiling                  |
| `ThreadCache::kDefaultTrimTargetBytes`      | 1 MiB                                                 | `ThreadCache`（thread\_cache.h）  | `kReuse` 软 trim 目标（best-effort）                       |
| `ThreadCache::InitialReservedQuotaBytes()`  | 207.75 KiB = Σ `kNumSizeClasses` 类 class\_size（当前 40） | `ThreadCache`（thread\_cache.h）  | 构造即预扣的「每类 1 对象」地板，同时是 budget 的下限                      |
...
- 字节上限：对象上限 × class\_size，即该类**单独**热起时的 quota 预约量。

| idx | 请求区间 (B)       | class\_size (B) | batch | 线性步长 | 对象上限 |    字节上限 |
| --: | -------------- | --------------: | ----: | ---: | ---: | ------: |
...
- **关联测试**: [tests/unit/test\_thread\_cache.cpp](../../tests/unit/test_thread_cache.cpp) / [tests/unit/test\_ammalloc.cpp](../../tests/unit/test_ammalloc.cpp) / [tests/unit/test\_quota\_policy.cpp](../../tests/unit/test_quota_policy.cpp)

- **架构总览**: [ammalloc\_design.md §5.1](ammalloc_design.md)

...
| 后台 RSS 回收（Scavenger + MADV\_DONTNEED） | 已实现                      |
...
所有表均为 `consteval` 编译期生成，附大量 `static_assert`（单调性、逆映射、对齐倍数、MAX\_TC\_SIZE 幂次边界）。
...
**入口单次 TLS 快照（`auto* tc = ptls;`）**：`am_malloc`/`am_free` 开头将 `thread_local ThreadCache* ptls` 拷贝到局部变量 `tc`，使整个函数只发生一次 TLS 读取（initial-exec 模型下为 FS 基址 + 偏移的间接寻址）。空值判定与随后的 `Allocate`/`Deallocate` 复用一个已验证的快照；若直接多次读 `ptls`，编译器无法证明两次读取之间其不被修改，只能保守地重复 TLS 取址。`tc == nullptr`（或大对象）时放行到慢路径，由 `CreateThreadCache()` 自行重读/创建 TLS，快慢路径互不干扰。该约定是单线程极速路径 \~3.8 ns 基线的前提之一。
...
| Scavenger 启动标志    | acquire / acq\_rel CAS | 保证单次启动        |
...
| Phase 2   | 完整 ABI 与自举                   | C malloc 族 / C++ new-delete 族、BootstrapAllocator、LD\_PRELOAD 目标、core 移除 spdlog  |
